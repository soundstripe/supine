...
from sqlalchemy.orm import joinedload, Session, InstrumentedAttribute
...
class Resource():
    ...
    update_params: Type[BaseModel] = None  # a pydantic model for update/PATCH parameters

    query_filter: Union[Type[Filter], Callable[[], None]] = lambda: None  # lambda: None for no filter
    ...
    @cached_property
    def result(self):
        model = create_model(self.model.__name__ + 'Result', __base__=BaseModel,
                             **{self.singular_name: (self.model, ...),
                                **{exp.plural_name: (List[exp.model], None) for exp in self.runtime_expansions}})
        model.__doc__ = self.model.__doc__
        return ApiResponse[model]

    @cached_property
    def list_result(self):
        model = create_model(self.model.__name__ + 'ListResult', __base__=BaseModel,
                             **{self.plural_name: (List[self.model], [])},)
        return PaginatedResponse[model]

    @cached_property
    def runtime_expansions(self):
        return [exp if isinstance(exp, Resource) else _resource_registry[exp] for exp in self.expansions]
