...
engine = sqlalchemy.create_engine('sqlite://?check_same_thread=False', poolclass=StaticPool, echo=True)
...
class CustomerOrm(OrmBase):
    ...
    territory_id: Mapper[int] = sqlalchemy.Column(sqlalchemy.ForeignKey('territory.territory_id'))

    territory = sqlalchemy.orm.relationship(TerritoryOrm)
    territories = sqlalchemy.orm.relationship(TerritoryOrm, uselist=True, overlaps='territory')
...
with S.begin() as session:
    ...
    session.add(CustomerOrm(customer_id=1, first_name='Sherlock', last_name='Holmes', territory_id=1))
